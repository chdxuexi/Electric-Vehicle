clc;
clear;
%% 按第一列排序，选择到达时间最小的那辆车进行调度充电
new_lie = sortArrayEachRow(carNum,new_lie);                         %调用sortArrayEachRow（）函数，先对每一行分别进行排序，并在每一行后面加上其相应的车辆编号1，2,3....100；
new_lie = sortrows(new_lie);                                        %对new_lie按照第一列的大小，对每行进行排序
priority = new_lie(:,pileNum+1);                                    %这样就得到了所有车辆调度顺序先后的数组
%% 计算每辆车到哪个充电桩进行充电，将其关系保存到 vehicle2station[]
vehicle2pile= zeros(1,pileNum);                                     %列数是充电桩编号，值是车辆编号，最短路径算法和这里不一样
vehicle2pile = [vehicle2pile;new_hangCharing(carNum+1:carNum+3,1:pileNum)];%这里是给vehicle2pile第四行加上了编号1,2,3，....pileNum
SumTimeOfPlies = zeros(1,pileNum);                                  %各个充电桩的时间累加矩阵
for i=1:carNum
    TheCar = priority(i);                                           %该轮循环，安排的车辆编号 = TheCar
    tempSingleCar = StoreEachsingleCar(:,:,TheCar);                 %该轮循环,该车的所有信息取出到tempSingleCar
        %先判断在j桩是否有车排队
